...
async def frage_claude(system: str, user_msg: str) -> str:
    ...
    # Key bereinigen: Zeilenumbrüche + Leerzeichen entfernen
    clean_key = ANTHROPIC_KEY.replace("\n", "").replace("\r", "").replace(" ", "").strip()
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post("https://api.anthropic.com/v1/messages", json=payload, headers=headers) as r:
                ...
                # Fehlerdetails ausgeben für besseres Debugging
                err_body = await r.text()
                print(f"[Claude API] Fehler {r.status}: {err_body[:300]}")
                return f"❌ API Fehler {r.status}: {err_body[:150]}"
